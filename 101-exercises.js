...
function endsWithVowel(string){
  var vowels = "aeiouAEIOU";
    if (vowels.indexOf(string[string.length-1])!==-1) {
      return true;
    }
  return false;
}
...
function startsAndEndsWithVowel(string){
  var vowels = "aeiouAEIOU";
    ...
    if ((vowels.indexOf(string[0]) !==-1)&&(vowels.indexOf(string[string.length-1])!==-1)) {
      return true;
    }
  return false;
}
...
function median(x){
  if (x.length%2===0){
    var medianIndex = x.length/2;
    return (x[medianIndex-1]+x[medianIndex])/2;
  } else{
    var medianIndex = Math.round(x.length/2);
...
function shortestString(x){
  ...
  var shortIndex = lengthArray.indexOf(Math.min(...lengthArray));
  return x[shortIndex];
}
...
function longestString(x){
  ...
  var longIndex = lengthArray.indexOf(Math.max(...lengthArray));
  return x[longIndex];
}
...
function totalNumberOfItems(x){
  ...
  for (var i =0; i<x["items"].length; i++){
    totalQuantity+= x["items"][i]["quantity"];
  }
...
function getAverageItemPrice(x){
  ...
  return totalPrice/x["items"].length;
}
...
function getAverageSpentPerItem(x){
  ...
  var totalQuantity = 0;
  for (var i =0; i<x["items"].length; i++){
    totalPrice+= x["items"][i]["price"]*x["items"][i]["quantity"];
    totalQuantity+=x["items"][i]["quantity"];
  }
...
function mostSpentOnItem(x){
  ...
  var highestIndex={};
  for (var i =0; i<x["items"].length; i++){
    var currentPrice =x["items"][i]["price"]*x["items"][i]["quantity"];
    if (currentPrice>highestPrice){
      highestPrice=currentPrice;
      highestIndex=x["items"][i];
    }
  }
  return highestIndex;
}
